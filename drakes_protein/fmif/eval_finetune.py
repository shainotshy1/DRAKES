...
for n in [1, 10, 15, 20]:
    for testing_model in model_to_test_list:
        ...
        for _step, batch in enumerate(loader_test):
            ...
            for item_idx in range(8):
                ...
                if args.decoding == 'cg':
                    ...
                    S_sp, prot_traj, clean_traj = noise_interpolant.sample(testing_model, X, mask, chain_M, residue_idx, chain_encoding_all,reward_model=reward_fn, n=n)
        ...
        with open(f'diffusion_analysis_new_7JJK_bon_{n}.csv', mode='w', newline='') as file:
